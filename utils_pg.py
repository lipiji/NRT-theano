...
def print_sent_dec(y_pred, y, y_mask, modules, consts, options, lvt_dict = None):
    ...
    max_num_docs = 3 if consts["batch_size"] > 3 else consts["batch_size"]
    is_unicode = options["is_unicode"]
    for idx_doc in range(max_num_docs):
        print (idx_doc + 1, "----------------------------------------------------------------------------------------------------")
        sent_true= ""
        for idx_word in range(int(max_y_words[idx_doc])):
            i = y[idx_word, idx_doc, 0] 
